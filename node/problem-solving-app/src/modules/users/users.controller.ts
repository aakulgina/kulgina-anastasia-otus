...
import { ApiBody, ApiCookieAuth, ApiExtraModels, ApiResponse, ApiTags } from '@nestjs/swagger';
...
@ApiTags('Users')
@ApiExtraModels(
    ChangeUserPasswordRequestDto, ChangeUserProfileRequestDto,
    CreateUserDto,
...
export class UsersController {
    ...
    async getUserProfile(
        ...
        const { id, userName, email, lastSeen } = this.usersService.getUserById(Number(userId));
    ...
    @ApiCookieAuth()
    @ApiBody({
        description: 'Although all the fields are optional, at least one have to be filled',
    ...
    @ApiResponse({
        status: 200,
        description: 'Profile data changed. Response body contains updated profile info',
        type: GetUserProfileResponseDto,
        example: {
            id: '123',
            userName: 'geek_cactus',
    ...
    @ApiResponse({
        status: 401,
        description: 'Only authorized users are able to change their own profiles',
    })
    @ApiResponse({
        status: 500,
        description: 'Any internal error occured',
    ...
    async changeUserProfileData(
        ...
        const { id, userName, email, lastSeen } = this.usersService.changeUserProfile(Number(userId), payload);
    ...
    @ApiResponse({
        status: 401,
        description: 'Only the authorized users themselves are able to delete their own profiles',
    })
    @ApiResponse({
        status: 500,
        description: 'Any internal error occured',
